...
export default function ChecklistTemplate() {
  ...
  useEffect(() => {
    const ckId = (location.state as any)?.checklistId;
    if (ckId) {
      loadFromChecklist(ckId);
      // remove state so re-navigation doesn't re-trigger automatically
      navigate('/checklist-template', { replace: true, state: {} });
    }
  // run once when component mounts or when location.state changes
  }, [location.state]);
  ...
  const loadFromChecklist = (ckId: string) => {
    ...
    const lvl = ck.level ? ck.level : 'N1';
    setTitle(`LPA ${lvl} – ${ck.name.toUpperCase()}`);
    setSideLabel(ck.category.toUpperCase());
    setItems(ck.items.map(item => ({
      id: item.id, area: ck.category.toUpperCase(), question: item.question, explanation: '',
  ...
  const handlePrint = () => {
    ...
    win.document.write(`<html><head><title>${title}</title><style>
      *{margin:0;padding:0;box-sizing:border-box}
      ...
      .page{page-break-after:always;border:2px solid #222;position:relative}
      ...
      .item-row{display:grid;border-bottom:1px solid #bbb;min-height:80px}
    ...
    // === FRENTE ===
    win.document.write(`<div class="page"><div class="side">${sideLabel}</div>`);
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 no-print">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button size="sm" variant="default" onClick={() => {
            ...
            // strip prefix LPA N# – from title for storage
            const match = title.match(/LPA\s+(N\d+)\s*[-–]\s*(.+)$/i);
            let ckName = title;
            let ckLevel: string | undefined = undefined;
            if (match) {
              ckLevel = match[1].toUpperCase();
              ckName = match[2].trim();
            }
            store.addChecklist({
              name: ckName,
              level: ckLevel,
              category: sideLabel,
              items: items.map(it => ({ id: it.id, question: it.question, type: 'ok_nok' as const })),
            });
