...
import { Plus, Trash2, QrCode, Pencil, Upload, Download, Search, Tag, History, FileSpreadsheet, RotateCcw } from 'lucide-react';
...
export default function Machines() {
  ...
  const [form, setForm] = useState({ name: '', code: '', sector: '', description: '' });
  ...
  const resetForm = () => setForm({ name: '', code: '', sector: '', description: '' });
  ...
  const handleResetMachines = () => {
    if (confirm('Deseja resetar todas as máquinas para os valores padrão? Esta ação não pode ser desfeita.')) {
      store.resetMachines();
      setMachines(store.getMachines());
      toast.success('Máquinas resetadas para valores padrão!');
    }
  };
    ...
    try {
      ...
      for (let i = 0; i < machs.length; i++) {
        ...
        const qrSvg = ReactDOMServer.renderToStaticMarkup(
          <QRCodeSVG value={`lpa-audit:machine:${m.id}`} size={130} level="M" />
        );
        ...
        label.innerHTML = `
          <div style="font-weight:900;font-size:32px;color:#1a237e;letter-spacing:3px;line-height:1;flex-shrink:0;">MAHLE</div>
          <div style="width:2px;height:78%;background:#9e9e9e;flex-shrink:0;"></div>
          <div style="flex:1;display:flex;align-items:center;justify-content:center;text-align:center;font-size:18px;font-weight:700;color:#1f1f1f;line-height:1.3;word-break:break-word;">
            ${labelText}
          </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" size="sm" onClick={handleResetMachines} title="Resetar máquinas para setores corretos"><RotateCcw className="mr-2 h-4 w-4" />Reset Máquinas</Button>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Select value={form.sector} onValueChange={v => setForm(f => ({ ...f, sector: v }))}>
                    <SelectTrigger className="w-full"><SelectValue placeholder="Selecione setor" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="Brochadeira">Brochadeira</SelectItem>
                      <SelectItem value="Chanfradeira">Chanfradeira</SelectItem>
                      <SelectItem value="Prensa Ressalto">Prensa Ressalto</SelectItem>
                      <SelectItem value="Inspeção Final">Inspeção Final</SelectItem>
                      <SelectItem value="Estampa Furo">Estampa Furo</SelectItem>
                      <SelectItem value="Prensa Curvar">Prensa Curvar</SelectItem>
                      <SelectItem value="Mandrila">Mandrila</SelectItem>
                      <SelectItem value="Fresa Canal">Fresa Canal</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
